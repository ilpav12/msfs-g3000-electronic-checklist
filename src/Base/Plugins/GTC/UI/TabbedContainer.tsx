...
import { TabbedContent } from "@microsoft/msfs-wtg3000-gtc";
...
export enum TabConfiguration {
  ...
  /** Four vertically stacked tabs on the right edge. */
  Right4 = "R4",

  /** Five vertically stacked tabs on the right edge. */
  Right5 = "R5",

  /** Four vertically stacked tabs on each of the left and right edges. */
  LeftRight4 = "LR4",

  /** Five vertically stacked tabs on each of the left and right edges. */
  LeftRight5 = "LR5",
...
export class TabbedContainer extends DisplayComponent<TabbedContainerProps> {
  ...
  private readonly selectedTabPosition: Subject<number> = Subject.create<number>(
    this.props.initiallySelectedTabPosition ?? 1,
  );
  ...
  private getTabEntry(position: number = this.selectedTabPosition.get()): TabEntry {
    ...
    if (!entry) {
      throw new Error(`TabbedContainer: Tab entry at position ${position} does not exist`);
    }
    return entry;
  }
  ...
  public render(): VNode {
    ...
    if (typeof this.props.class === "object") {
      const reservedClasses = [
        ...
        "gtc-tabbed-container-r4",
        "gtc-tabbed-container-r5",
        "gtc-tabbed-container-lr4",
        "gtc-tabbed-container-lr5",
    ...
    return (
      <div class={cssClass}>
        ...
        <div class="gtc-tab-content-container">{this.tabEntries.map((entry) => entry?.contentNode)}</div>
      </div>
    );
  }
  ...
  private validateChildren(): void {
    ...
    (ArrayUtils.flat(this.props.children, Infinity) as (VNode | string | number | Subscribable<any>)[]).forEach(
      (child) => {
        ...
        if (
          (this.props.configuration === (TabConfiguration.Left4 || TabConfiguration.Right4) && position > 4) ||
          (this.props.configuration === (TabConfiguration.Left5 || TabConfiguration.Right5 || TabConfiguration.Top) &&
            position > 5) ||
          (this.props.configuration === TabConfiguration.LeftRight4 && position > 8) ||
          (this.props.configuration === TabConfiguration.LeftRight5 && position > 10)
        ) {
          throw new Error(
            `TabbedContainer: position passed to TabbedContent (${position}) is out of bounds for the tab configuration (${this.props.configuration})`,
          );
        }
  ...
  private renderTab(
    ...
    if (this.props.configuration !== TabConfiguration.Top) {
      classes.push(
        this.isTabRight(position) ? "gtc-tab-scooped-corner-vert-right" : "gtc-tab-scooped-corner-vert-left",
      );
    }
    const topClasses: string = [
      ...classes,
      this.props.configuration === TabConfiguration.Top
        ? "gtc-tab-scooped-corner-hor-left"
        : "gtc-tab-scooped-corner-vert-top",
    ].join(" ");
    const bottomClasses: string = [
      ...classes,
      this.props.configuration === TabConfiguration.Top
    ...
    isEnabled.sub((enabled) => {
      rootCssClass.toggle("gtc-tab-disabled", !enabled);
    }, true);
    isSelected.sub((selected) => {
      rootCssClass.toggle("gtc-tab-selected", selected);
    }, true);

    return (
      <div ref={ref} data-position={position} class={rootCssClass}>
  ...
  private generateTabClasses(position: number): SetSubject<string> {
    ...
    if (this.props.configuration === TabConfiguration.Top) {
      ...
      if (position === 1) {
        cssClass.add("gtc-tab-hor-start");
      }
      if (position === 5) {
        cssClass.add("gtc-tab-hor-end");
      }
    } else {
      const height: number =
        this.props.configuration === TabConfiguration.Left5 ||
        this.props.configuration === TabConfiguration.Right5 ||
        this.props.configuration === TabConfiguration.LeftRight5
          ? 5
          : 4;
      cssClass.add(this.isTabRight(position) ? "gtc-tab-right" : "gtc-tab-left");
      if (position === 1 || position % (height + 1) === 0) {
        cssClass.add("gtc-tab-vert-start");
      }
      if (position % height === 0) {
        cssClass.add("gtc-tab-vert-end");
      }
    }
  ...
  private renderContent(contentNode: VNode, isSelected: Subscribable<boolean>): VNode {
    return <div class={isSelected.map((selected) => `gtc-tab-content${selected ? "" : " hidden"}`)}>{contentNode}</div>;
  }
  ...
  private isTabRight(position: number): boolean {
    return (
      (this.props.configuration === TabConfiguration.LeftRight4 && position >= 5) ||
      (this.props.configuration === TabConfiguration.LeftRight5 && position >= 6) ||
      this.props.configuration === TabConfiguration.Right4 ||
      this.props.configuration === TabConfiguration.Right5
    );
  }
}
