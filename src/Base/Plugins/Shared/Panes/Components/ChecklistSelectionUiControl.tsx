...
export interface ChecklistSelectionProps<Names, Category>
  extends ChecklistUiControlProps {
  /** The event bus */
  bus: EventBus;
  /** The pane paneIndex. */
...
export class ChecklistCategorySelectionControl<
  Names,
  Category,
> extends ChecklistSelectionUiControl<Names, Category> {
  private readonly items = ArraySubject.create(
    this.props.repo.getChecklistCategories(),
  );
...
export class ChecklistSelectionControl<
  Names,
  Category,
> extends ChecklistSelectionUiControl<Names, Category> {
  private readonly items = ArraySubject.create(
    ...
    this.props.bus
      ...
      .handle((event) => {
        if (
          ...
          this.items.set(
            this.props.repo
              .getChecklistsByCategory(event.newActiveChecklistCategory)
              .map((v) => v.name),
          );
        }
      });
